...
window.gisMap = config => ({
    ...
    async loadComplaints() {
        ...
        (data.features || []).forEach(feature => {
            ...
            marker.bindPopup(`<div class="map-popup"><strong>${escapeHtml(p.title)}</strong><div>${escapeHtml(p.category_label || '')}${p.region ? ` · ${escapeHtml(p.region)}` : ''}</div><p>${escapeHtml(p.address || '')}</p><a href="${escapeHtml(p.url || '#')}">Lihat detail laporan</a></div>`);
